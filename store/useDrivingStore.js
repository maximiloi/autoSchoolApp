...
export const useDrivingStore = create(
  immer((set, get) => ({
    sessions: [],
    isLoading: false,
    isSaving: false,
    ...
    fetchDrivingData: async (groupId, setGroup) => {
      ...
      try {
        ...
        setGroup(groupData);
        set((state) => {
          state.sessions = normalizeDrivingSessions(sessionsData);
        });
      } catch (error) {
    ...
    updateSlot: async (studentId, date, status) => {
      ...
      set((state) => {
        if (!state.sessions[studentId]) {
          state.sessions[studentId] = {};
        }
        state.sessions[studentId][date] = status;
      });
      return { error: null };
    },
