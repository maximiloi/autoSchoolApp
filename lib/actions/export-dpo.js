...
export async function exportDpoStudentsToBuffer(companyId, year, isFirstHalf) {
  const session = await auth();
  if (!session) throw new Error('User is not authenticated');
  if (!companyId) throw new Error('Company ID is required');

  const startDate = new Date(`${year}-01-01`);
  const isFirstHalfBool = isFirstHalf === 'true';
  const endDate = isFirstHalfBool ? new Date(`${year}-06-30`) : new Date(`${year}-12-31`);
  ...
  const students = await prisma.student.findMany({
    where: {
      ...
      group: {
        is: {
          startTrainingDate: { gte: startDate },
          endTrainingDate: { lte: endDate },
        },
      },
    },
    include: { group: true },
